  ...
  return async (req: CustomRequest, res: CustomResponse, next: NextFunction) => {
    try {
      ...
      // try {
        ...
        if (modelName === "DraftPost") {
          if (!userId) {
            throw makeError(400, "userId is required for DraftPost");
          }

          if (!mongoose.Types.ObjectId.isValid(userId)) {
            next(makeError(400, "userId is required for DraftPost")); 
          }

          const postAuthor = await UserModel.findById(userId).lean();
          if (!postAuthor) next(makeError(404, "The user with the given ID was not found"));
          
          matchConditions.postAuthorId = new mongoose.Types.ObjectId(userId);
        ...
        const generateProjectStage = (model: CustomModel) => {
          ...
          if (modelName === "DraftPost") {
            ...
            projectFields.publishedPostId = 1,
            projectFields.tags = 1,
            projectFields.category = 1,
            projectFields.status = 1,
            projectFields.deletionStatus = 1,
            projectFields.lastEditedAt = 1
            ...
            projectFields.tags = 1,
            projectFields.category = 1
            projectFields.currentDraftId = 1,
            projectFields.likeCount = 1,
            projectFields.commentCount = 1,
            projectFields.status = 1,
            projectFields.deletionStatus = 1,
            projectFields.publishedAt = 1,
            projectFields.deletedAt = 1,
            projectFields.archivedAt = 1
